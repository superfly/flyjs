...
const parseURL = process.env.FLY_ENV === "test"
  ? parseUrlWithRemapping
  : parseUrl;
  ...
  (
    ...
    handler(rt, bridge, parsedUrl, init, body)
      .then((resp) => {
        ...
        rt.log(
          "info",
          `[fetch] ${init.method} ${url} ${resp.status} ${
...
export function dispatchFetchResponse(
  cb: IvmCallback,
  response: ResponseInit & { url: string },
): ResponseInit {
  ...
  cb.applyIgnored(
    null,
    // @ts-ignore comment
    [null, new ivm.ExternalCopy(init).copyInto({ release: true }), body],
  );
