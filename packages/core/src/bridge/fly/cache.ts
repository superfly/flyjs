...
const errCacheStoreUndefined = new Error(
  "cacheStore is not defined in the config.",
);
const errCacheNotifierUndefined = new Error(
  "cacheNotifier is not defined in the config",
);
...
registerBridge("flyCacheGetMulti", function cacheGet(
  ...
  bridge.cacheStore
    ...
    .then((result) => {
      let byteLength = 0;
      const toTransfer: Array<null | ivm.Copy<ArrayBuffer>> = result.map(
        (b) => {
