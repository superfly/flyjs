...
registerBridge("fly.Image.metadata", async function imageMetadata(
  ...
  return new ivm.ExternalCopy(extractMetadata(meta)).copyInto(
    { release: true },
  );
});
...
async function scale(this: Sharp, ...args: any[]) {
  ...
  const ignoreAspectRatio = typeof opts === "object" &&
    opts.ignoreAspectRatio === true;
  const withoutEnlargement = typeof opts === "object" &&
    opts.allowEnlargement === false;

  if (!width || !height) {
    const meta = await this.metadata();
    if (!width && height) {
      width = relativeDimension(
        width,
        meta.width || 0,
        height,
        meta.height || 0,
