...
export const streamManager = {
  ...
  subscribe(rt: Runtime, id: number | string, cb: ivm.Reference<() => void>) {
    ...
    if (!info) {
      return cb.applyIgnored(
        null,
        // @ts-ignore comment
        ["stream not found or destroyed after timeout"],
      );
    }
  ...
  read(rt: Runtime, id: number | string, cb: ivm.Reference<() => void>) {
    ...
    if (!info) {
      return cb.applyIgnored(
        null,
        // @ts-ignore comment
        ["stream closed, not found or destroyed after timeout"],
      );
    }
